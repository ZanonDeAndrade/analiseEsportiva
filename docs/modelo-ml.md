...
O backtest walk-forward treina, para cada alvo, apenas com registros cujo instante seja estritamente anterior ao alvo. Assim, outro jogo do mesmo dia não pode vazar para o histórico. O `runBacktest` usa **estado incremental** (`incrementalModel.ts`): como o modelo de frequências + perfis é aditivo, acumular uma partida por vez equivale a treinar do zero sobre todo o passado, eliminando o retreino O(n²) — o dataset completo (6.406 partidas) roda em ~2,5 s, com a duração reportada em `durationMs`. As partidas da mesma data são previstas com o estado anterior à data (look-back estrito) e só depois entram no estado; um teste compara a saída com uma implementação de referência (retreino) para garantir equivalência exata.
...
## Dados insuficientes e confiança (ETAPA 17)

O status `dados_insuficientes` é preservado e suas **causas são diferenciadas** (`models/confidence.ts`): dado ausente, valor zero legítimo (nunca confundido com ausência), amostra insuficiente, competição sem histórico, equipe nova e feature indisponível — com critérios mínimos configuráveis por equipe, competição, mercado, tipo de feature e janela temporal. A **confiança não é um rótulo arbitrário**: `computeConfidence` a calcula a partir de seis fatores — tamanho da amostra, estabilidade entre folds temporais, calibração, distância em relação ao baseline, disponibilidade de features e incerteza estatística — e sempre acompanha o aviso de que **não é promessa de acerto** nem recomendação de aposta.
